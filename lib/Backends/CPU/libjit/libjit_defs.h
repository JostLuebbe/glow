...
#define AT(tensor, dims, numDims, indices, numIndices) tensor[get_element_ptr(tensor, dims, numDims, indices, numIndices)]
...
inline void AdduFloat8(float *p, float8 v) { StoreuFloat8(p, LoaduFloat8(p) + v); }

/// \returns the index of the element at x,y,z,w,q,r.
inline dim_t libjit_getXYZWQR(const dim_t *dims, dim_t x, dim_t y, dim_t z, dim_t w, dim_t q, dim_t r) {
    return (x * dims[1] * dims[2] * dims[3] * dims[4] * dims[5]) + (y * dims[2] * dims[3] * dims[4] * dims[5]) + (z * dims[3] * dims[4] * dims[5]) +
           (w * dims[4] * dims[5]) + (q * dims[5]) + r;
}

/// \returns the index of the element at x,y,z,w,q.
inline dim_t libjit_getXYZWQ(const dim_t *dims, dim_t x, dim_t y, dim_t z, dim_t w, dim_t q) {
    return (x * dims[1] * dims[2] * dims[3] * dims[4]) + (y * dims[2] * dims[3] * dims[4]) + (z * dims[3] * dims[4]) + (w * dims[4]) + q;
}
...
inline dim_t libjit_getXYZ(const dim_t *dims, dim_t x, dim_t y, dim_t z) { return (x * dims[1] * dims[2]) + (y * dims[2]) + z; }

/// \returns the index of the element at x,y.
inline dim_t libjit_getXY(const dim_t *dims, dim_t x, dim_t y) { return (x * dims[1]) + y; }

inline int8_t libjit_clip(int32_t val) { return (int8_t)MIN(MAX(val, -128), 127); }

/// Scales a 32-bit integer using the integer shift-mult-shift method.
/// See QuantizationTransform32To8 for more details.
inline int32_t libjit_scale_i32i8(int32_t input, int32_t pre, int32_t post, int32_t scale, int32_t offset) {
...
#ifdef _WIN32
#define libjit_aligned_malloc(p, a, s) (((*(p)) = _aligned_malloc((s), (a))), *(p) ? 0 : errno)
