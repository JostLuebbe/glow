...
static void loadImagesAndPreprocess(const std::vector<std::string> &filenames, float *&resultT, size_t *resultDims) {
    ...
    size_t resultSizeInBytes = numImages * 3 * DEFAULT_HEIGHT * DEFAULT_WIDTH * sizeof(float);
        ...
        for (unsigned z = 0; z < 3; z++) {
            for (unsigned y = 0; y < dims[1]; y++) {
                for (unsigned x = 0; x < dims[0]; x++) {
                    resultT[getXYZW(resultDims, n, 2 - z, x, y)] = imageT[getXYZ(dims, x, y, z)];
                }
            }
        }
    }
...
static void dumpInferenceResults(const BundleConfig &config, uint8_t *mutableWeightVars) {
    const SymbolTableEntry &outputWeights = getMutableWeightVar(config, "gpu_0_softmax");
    int maxIdx = 0;
    float maxValue = 0;
...
static uint8_t *initMutableWeightVars(const BundleConfig &config) {
    ...
    const SymbolTableEntry &inputGPUDataVar = getMutableWeightVar(config, "gpu_0_data");
...
int main(int argc, char **argv) {
    ...
    uint8_t *constantWeightVarsAddr = initConstantWeights("resnet50.weights.bin", resnet50_config);
