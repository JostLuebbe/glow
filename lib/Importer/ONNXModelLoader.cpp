...
llvm::cl::list<std::string, std::vector<std::string>>
    onnxDefineSymbolOpt("onnx-define-symbol", llvm::cl::ZeroOrMore, llvm::cl::location(onnxDefineSymbol),
                        llvm::cl::desc("Define (replace) the undefined symbols from the tensor descriptions\n"
...
Expected<std::unordered_map<std::string, dim_t>> getSymbolMap() {
    ...
    for (const auto &str : onnxDefineSymbol) {
        ...
        dim_t value;
        RETURN_ERR_IF_NOT(!strPair.second.getAsInteger(0, value), strFormat("ONNX defined symbol value '%s' is invalid.", strPair.second.data()));
...
Expected<std::vector<dim_t>> getProtoShape(const ONNX_NAMESPACE::TensorShapeProto &shapeProto) {
    ...
    for (auto d : shapeProto.dim()) {
        if (d.has_dim_value()) {
            ...
            if (symbolMap.count(symbolName)) {
                ...
                RETURN_ERR(strFormat("ONNX model symbol '%s' is undefined. Define the symbol with the "
...
Error onnxTensorDataTypeToElemKind(int32_t onnxType, ElemKind *elemTy) {
    if (onnxType == ONNX_NAMESPACE::TensorProto::FLOAT) {
        ...
        *elemTy = ElemKind::BoolTy;
        ...
        RETURN_ERR(strFormat("Don't know how to convert ONNX tensor data type %d to ElemKind", onnxType));
    }
}
...
Expected<std::string> getAttrFromDocString(const std::string &attr, const std::string &docStr) {
...
Expected<std::pair<float, int32_t>> getQuantParamsFromDocString(const std::string &docStr) {
    std::string scaleStr;
    ASSIGN_VALUE_OR_RETURN_ERR(scaleStr, getAttrFromDocString(qScaleSignifier, docStr));
    float scale = std::strtof(scaleStr.c_str(), NULL);

    std::string offsetStr;
    ASSIGN_VALUE_OR_RETURN_ERR(offsetStr, getAttrFromDocString(qOffsetSignifier, docStr));
    int32_t offset;
...
template <bool IsInteger, typename T> struct AttributeRetriever {
    static Expected<T> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader &loader);
};

/// Specialization for std::vector<float>.
template <> struct AttributeRetriever<false, std::vector<float>> {
    static Expected<std::vector<float>> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader & /* unused */) {
...
template <> struct AttributeRetriever<false, std::vector<NodeValue>> {
    static Expected<std::vector<NodeValue>> get(const ONNX_NAMESPACE::AttributeProto *attr, ProtobufLoader &loader) {
...
template <> struct AttributeRetriever<false, NodeValue> {
    static Expected<NodeValue> get(const ONNX_NAMESPACE::AttributeProto *attr, ProtobufLoader &loader) {
...
template <typename T> struct AttributeRetriever<false, std::vector<T>> {
    static Expected<std::vector<T>> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader & /* unused */) {
...
template <typename T> struct AttributeRetriever<true, T> {
    static Expected<T> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader & /* unused */) { return loadInt(attr); }
};

/// Specialization for LengthsMode.
template <> struct AttributeRetriever<false, LengthsMode> {
    static Expected<LengthsMode> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader & /* unused */) {
        std::string str;
        ASSIGN_VALUE_OR_RETURN_ERR(str, loadStr(attr));
        if (str == "AllOne") {
...
template <> struct AttributeRetriever<false, FusedActivation> {
    static Expected<FusedActivation> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader & /* unused */) {
        std::string str;
        ASSIGN_VALUE_OR_RETURN_ERR(str, loadStr(attr));
        if (str == "NONE") {
...
template <> struct AttributeRetriever<false, ConvolutionLayout> {
    static Expected<ConvolutionLayout> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader & /* unused */) {
        std::string str;
        ASSIGN_VALUE_OR_RETURN_ERR(str, loadStr(attr));
        if (str == "NHWC") {
...
template <> struct AttributeRetriever<false, PaddingMode> {
    static Expected<PaddingMode> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader & /* unused */) {
        std::string str;
        ASSIGN_VALUE_OR_RETURN_ERR(str, loadStr(attr));
        if (str == "CONSTANT") {
...
template <> struct AttributeRetriever<false, float> {
    static Expected<float> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader & /* unused */) { return loadFloat(attr); }
};

/// Specialization for std::string.
template <> struct AttributeRetriever<false, std::string> {
    static Expected<std::string> get(const ONNX_NAMESPACE::AttributeProto *attr, const ProtobufLoader & /* unused */) { return loadStr(attr); }
};

/// Forwards to the correct AttributeRetriever specialization.
template <typename T> Expected<T> loadAttribute(const ONNX_NAMESPACE::AttributeProto *attr, ProtobufLoader &loader) {
    RETURN_ERR_IF_NOT(attr, "No such attribute");
    return AttributeRetriever<std::numeric_limits<T>::is_integer, T>::get(attr, loader);
}

} // namespace

using ArgumentDictionaryTy = std::unordered_map<std::string, const ONNX_NAMESPACE::AttributeProto *>;
...
Expected<Type> parseTypeFromDocString(const std::string &str, llvm::ArrayRef<dim_t> dims, bool useGlowCustomOps) {
    float scale = 1.0;
    int32_t offset = 0;
    ...
    if (useGlowCustomOps) {
        ...
        ASSIGN_VALUE_OR_RETURN_ERR(elemKindStr, getAttrFromDocString(elemKindSignifier, str));
        elemKind = Type::getElementKindFromName(elemKindStr);

        if (isQuantizedElemKind(elemKind)) {
            std::pair<float, int32_t> scaleOffsetPair;
            ASSIGN_VALUE_OR_RETURN_ERR(scaleOffsetPair, getQuantParamsFromDocString(str));
...
void glow::setOnnxDefineSymbol(const std::vector<std::string> &strs) { onnxDefineSymbol = strs; }
...
void glow::fillPlaceholders(const ONNX_NAMESPACE::GraphProto &inputGroup, PlaceholderBindings *bindings, std::vector<Tensor> *partialTensorPayloads,
                            bool usingGlowCustomOps) {
    for (const auto &tensorProto : inputGroup.initializer()) {
        auto *tensor = bindings->get(bindings->getPlaceholderByName(tensorProto.name()));
        CHECK(tensor);
        ...
        if (loadedSize != fullSize) {
            if (partialTensorPayloads) {
                VLOG(1) << "Loading " << tensorProto.name() << " as a partial tensor: partial size=" << tensor->getType().toString()
                        << " full size=" << fullType.toString();
                Tensor fullTensor(tensor->getUnsafePtr(), &fullType, tensor->getSizeInBytes());
                ...
                *tensor = std::move(fullTensor);
                ...
                // pad with 0
                VLOG(1) << "Loading and padding " << tensorProto.name() << " as a partial tensor: partial size=" << tensor->getType().toString()
                        << " full size=" << fullType.toString();
                Tensor fullTensor(&fullType);
                std::memcpy(fullTensor.getUnsafePtr(), tensor->getUnsafePtr(), tensor->getSizeInBytes());
                std::memset(fullTensor.getUnsafePtr() + tensor->getSizeInBytes(), 0, fullTensor.getSizeInBytes() - tensor->getSizeInBytes());
                *tensor = std::move(fullTensor);
            }
        }
    }
...
void glow::fillPlaceholders(const std::string &fileName, PlaceholderBindings *bindings, std::vector<Tensor> *partialTensorPayloads,
                            bool usingGlowCustomOps) {
    const ONNX_NAMESPACE::GraphProto &inputGroup = parseOnnxFile(fileName);
    fillPlaceholders(inputGroup, bindings, partialTensorPayloads, usingGlowCustomOps);
}

/// Loads tensor \p T from the input \p in.
Error glow::loadTensor(const ONNX_NAMESPACE::TensorProto &in, Tensor *T, bool useGlowCustomOps) {
    std::vector<dim_t> dim;
    for (auto d : in.dims()) {
        dim.push_back(d);
    }
    ...
    if (in.data_type() == ONNX_NAMESPACE::TensorProto::FLOAT) {
        ...
        if (in.float_data_size() > 0) {
            ...
            RETURN_ERR("Unsupported Tensor format.", ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_DATATYPE);
        }
        ...
        Type ty;
        ASSIGN_VALUE_OR_RETURN_ERR(ty, parseTypeFromDocString(in.doc_string(), dim, useGlowCustomOps));
        T->reset(ty);
        ...
        RETURN_ERR(strFormat("Unsupported tensor data type: %u", static_cast<unsigned>(in.data_type())),
                   ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_DATATYPE);
    }
    return Error::success();
}

Error ONNXModelLoader::setTensorType(const ONNX_NAMESPACE::ValueInfoProto &in, Tensor *T) {
    auto type = in.type();
    ...
    if (useGlowCustomOps_) {
        std::string elemKindStr;
        ASSIGN_VALUE_OR_RETURN_ERR(elemKindStr, getAttrFromDocString(elemKindSignifier, in.doc_string()));
        kind = Type::getElementKindFromName(elemKindStr);
        if (isQuantizedElemKind(kind)) {
            std::pair<float, int32_t> scaleOffsetPair;
            ASSIGN_VALUE_OR_RETURN_ERR(scaleOffsetPair, getQuantParamsFromDocString(in.doc_string()));
            std::tie(scale, offset) = scaleOffsetPair;
        }
    } else {
        // Retrieve the ElemKind from the ONNX type, including considerations for
        // whether the datatype is quantized.
        RETURN_IF_ERR(onnxTensorDataTypeToElemKind(type.tensor_type().elem_type(), &kind));
    }
    ...
    if (isQuantizedElemKind(kind)) {
        assert(useGlowCustomOps_ && "Quantized loading not fully supported without custom Glow ops.");
...
Error ONNXModelLoader::loadInputs(ONNX_NAMESPACE::GraphProto &net, bool loadInputsAsPlaceholders) {
    for (const auto &in : net.input()) {
        ...
        if (loadInputsAsPlaceholders) {
            ...
            if (useGlowCustomOps_) {
                ASSIGN_VALUE_OR_RETURN_ERR(isTrainable, getAttrFromDocString(trainableSignifier, in.doc_string()));
                ASSIGN_VALUE_OR_RETURN_ERR(layout, getAttrFromDocString(layoutSignifier, in.doc_string()));
            }

            Placeholder *placeholder;
            ASSIGN_VALUE_OR_RETURN_ERR(
                placeholder, createAndRegisterPlaceholder(in.name(), &T.getType(), staticInputs_.count(in.name()), isTrainable != "0", layout));
...
bool ONNXModelLoader::hasMultidirectionalBroadcast(const llvm::StringRef typeName) {
    // Before opset 7, broadcasting was unidirectional.
    if (opsetVersion_ > 6) {
        if ((typeName == "Add") || (typeName == "Sub") || (typeName == "Mul") || (typeName == "Div")) {
            return true;
        }
...
Expected<ElemKind> ONNXModelLoader::convertTensorProtoDataType(ONNX_NAMESPACE::TensorProto_DataType t) {
    switch (t) {
...
Error ONNXModelLoader::setVersion(ONNX_NAMESPACE::ModelProto MP) {
    ...
    opsetVersion_ = 0;
    RETURN_ERR_IF_NOT(irVersion_ >= 3, "This ONNX model with ir_version < 3 is too old to be supported.",
    ...
    RETURN_ERR_IF_NOT(opsetVersion_ > 0, "The opset of this ONNX model is not supported.");
    return Error::success();
}

Expected<ONNX_NAMESPACE::ModelProto> ONNXModelLoader::loadProto(google::protobuf::io::ZeroCopyInputStream &iStream) {
    ...
    RETURN_ERR_IF_NOT(parseNet, "Failed to parse ModelProto", ErrorValue::ErrorCode::MODEL_LOADER_INVALID_PROTOBUF);
    return MP;
}

Expected<ONNX_NAMESPACE::ModelProto> ONNXModelLoader::loadProto(const void *onnxModel, size_t onnxModelSize) {
    google::protobuf::io::ArrayInputStream arrayStream(onnxModel, onnxModelSize);
    return loadProto(arrayStream);
}

Expected<ONNX_NAMESPACE::ModelProto> ONNXModelLoader::loadProto(const std::string &filename) {
    std::ifstream ff(filename, std::ios::in | std::ios::binary);
    RETURN_ERR_IF_NOT(ff, strFormat("Can't find the model or network files for %s.", filename.c_str()),
    ...
    if (filename.find(".onnxtxt") != std::string::npos) {
        std::string str((std::istreambuf_iterator<char>(ff)), std::istreambuf_iterator<char>());
        ONNX_NAMESPACE::ModelProto MP;
...
template <typename T> T ceil(float val) { return (val - (T)val) > 0 ? (T)(val + 1) : (T)val; }
...
Expected<Pads> getPads(ArgumentDictionaryTy &dict, llvm::ArrayRef<unsigned_t> kdim, llvm::ArrayRef<unsigned_t> sdim,
                       llvm::ArrayRef<unsigned_t> idim) {
    if (dict.count("pads")) {
        if (dict.at("pads")->ints_size() == 2) { // For maxPool1D
            return Pads({0, (unsigned_t)dict.at("pads")->ints(0), 0, (unsigned_t)dict.at("pads")->ints(1)});
        }
...
/// \p sdim: stride sizes (HW)
/// \p idim: input sizes (HW)
static Expected<Pads> getConvTransposePadsfromOutput(ArgumentDictionaryTy &dict, llvm::ArrayRef<unsigned_t> kdim, llvm::ArrayRef<unsigned_t> sdim,
                                                     unsigned_t dilation, llvm::ArrayRef<unsigned_t> idim, llvm::ArrayRef<unsigned_t> odim) {

    llvm::SmallVector<unsigned_t, 2> pdim(2); // Total Paddding, HW.
    for (size_t i = 0, e = pdim.size(); i < e; i++) {
        pdim[i] = sdim[i] * (idim[i] - 1) /* + output_padding[0]*/ + ((kdim[i] - 1) * dilation + 1) - odim[i];
    }
...
Error ONNXModelLoader::loadConstant(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    /*
      output: "Parameter6"
      name: "Parameter6"
      op_type: "Constant"
    ...
    */
    ...
    RETURN_ERR_IF_NOT((type == ONNX_NAMESPACE::AttributeProto::TENSOR || type == ONNX_NAMESPACE::AttributeProto::INTS),
                      "Only Tensor type constants are supported.", ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_DATATYPE);
...
template <typename T, typename datatype = ssize_t> static void helperSetter(Constant *constT, std::vector<datatype> &vec) {
...
Error ONNXModelLoader::loadSlice(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    ...
    if (this->opsetVersion_ >= 10) {
        ...
        if (op.input_size() > 3) {
            ...
            RETURN_ERR_IF_NOT(op.input_size() == 5, "Steps is not currently supported.");
        }
    } else {
    ...
    RETURN_ERR_IF_NOT((starts.size() == ends.size()), "Slice: 'starts' and 'ends' arrays must have the same size.");
    ...
    RETURN_ERR_IF_NOT(axes.size() == starts.size(), "'axes' and 'starts' must be the same size.");
    RETURN_ERR_IF_NOT(starts.size() == ends.size(), "'starts' and 'ends' must be the same size.");
    for (size_t i = 0; i < axes.size(); i++) {
        ssize_t newStart = starts[i];
        ssize_t newEnd = ends[i];
        ssize_t axisId = axes[i];
        RETURN_ERR_IF_NOT((axisId >= 0) && (axisId < ssize_t(numDims)), "Axes indexes must be within the input tensor range.");
        ...
        if (newStart < 0) {
            ...
            RETURN_ERR_IF_NOT(newStart >= 0, "Slice: final start index should never be negative.");
        }
        if (newEnd < 0) {
            newEnd = ssize_t(dims[axisId]) + newEnd;
            RETURN_ERR_IF_NOT(newEnd >= 0, "Slice: final end index should never be negative.");
        }
...
Error ONNXModelLoader::loadConv1D(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    // Load the attributes
    ...
    unsigned_t dilation = dict.count("dilations") ? dict.at("dilations")->ints(0) : 1;
    ...
    TransposeNode *filterTransposeNode = G_->createTranspose(opName, filterValue, NCHW2NHWC);
    ...
    auto outSz = calculateConvPoolOutputDims(idim.h, idim.w, kernelShape, strides, pads, dilation);
    std::array<dim_t, 4> outDims = {{idim.n, outSz.first, outSz.second, depth}};
    auto outTy = mod_.uniqueType(ElemKind::FloatTy, outDims);
    auto *node = G_->createConv(opName, tr, filterTransposeNode, bias, outTy, kernelShape, strides, pads, group, dilation);
...
Error ONNXModelLoader::loadConv(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    ...
    if (dict.count("dilations")) {
        ...
        ASSIGN_VALUE_OR_RETURN_ERR(dilations, getShape<unsigned_t>(dict["dilations"]));
        RETURN_ERR_IF_NOT(dilations.size() == 2, "Conv: dilations must be specified for 2 axes.");
        RETURN_ERR_IF_NOT(dilations[1] == dilations[0], "Conv: different dilation values along different axes "
    ...
    if (dict.count("kernel_shape")) {
        ...
        ASSIGN_VALUE_OR_RETURN_ERR(kernelShapeAttribute, getShape<unsigned_t>(dict["kernel_shape"]));
        RETURN_ERR_IF_NOT((kernelShape[0] == kernelShapeAttribute[0] && kernelShape[1] == kernelShapeAttribute[1]),
...
Error ONNXModelLoader::loadTensorwiseQuantizedConvolution(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    ...
    ASSIGN_VALUE_OR_RETURN_ERR(kernels, getShape<unsigned_t>(dict["kernel_shape"]));
    ...
    std::array<dim_t, 4> outDims = {{idim.n, outSz.first, outSz.second, biasValue.dims()[0]}};
    auto outTy = mod_.uniqueType(ElemKind::Int8QTy, outDims, outScale, outOffset);

    auto *node = G_->createConv(opName, input, filterValue, biasValue, outTy, kernels, strides, pads, groups);

    return addNodeAsOutput(op, node);
}

Error ONNXModelLoader::loadChannelwiseQuantizedConvolution(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    ...
    auto *node =
        G_->createChannelwiseQuantizedConv(opName, input, filterValue, biasValue, scalesValue, offsetsValue, outTy, kernels, strides, pads, groups);

    return addNodeAsOutput(op, node);
}

Error ONNXModelLoader::loadConvTranspose(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    // Load the attributes
    std::vector<unsigned_t> strides(2, 1);
    if (dict.count("strides")) {
    ...
    if (dict.count("dilations")) {
        ...
        ASSIGN_VALUE_OR_RETURN_ERR(dilations, getShape<unsigned_t>(dict["dilations"]));
        RETURN_ERR_IF_NOT(dilations.size() == 2, "ConvTranspose: dilations must be specified for 2 axes.");
        RETURN_ERR_IF_NOT(dilations[1] == dilations[0], "ConvTranspose: different dilation values along different axes "
                                                        "are not supported currently. values must be same.");
        dilation = dilations[0];
    }
    ...
    // weights in the format CRSK. ONNX stores the operators as KCRS.
    // C - output_depth, R - filter_height, S - filter_width, K - input_depth.
    TransposeNode *filterTransposeNode = G_->createTranspose(opName, filterValue, CNHW2NHWC /* flip matrix */);
    ...
    if (dict.count("kernel_shape")) {
        std::vector<unsigned_t> kernelShapeAttribute;
        ASSIGN_VALUE_OR_RETURN_ERR(kernelShapeAttribute, getShape<unsigned_t>(dict["kernel_shape"]));
        RETURN_ERR_IF_NOT((kernels[0] == kernelShapeAttribute[0] && kernels[1] == kernelShapeAttribute[1]),
                          "The 'kernel_shape' attribute is not consistent with the actual "
                          "convolution kernel shape.");
        (void)kernelShapeAttribute; // Avoids compilation warning in release mode.
    }
    ...
    if (dict.count("output_shape")) {
        ...
        ASSIGN_VALUE_OR_RETURN_ERR(outShape, getShape<unsigned_t>(dict["output_shape"]));
        ASSIGN_VALUE_OR_RETURN_ERR(pads, getConvTransposePadsfromOutput(dict, kernels, strides, dilation, idimHW, outShape));
        outSz = {outShape[0], outShape[1]};

        std::pair<dim_t, dim_t> outSzTest = calculateConvTransposeOutputDims(idim.h, idim.w, kernels, strides, pads, dilation);
        RETURN_ERR_IF_NOT((outShape[0] == outSzTest.first), "Expected/calculated pads don't match");
        RETURN_ERR_IF_NOT((outShape[1] == outSzTest.second), "Expected/calculated pads don't match");
    } else {
        if (dict.count("output_padding")) {
            RETURN_ERR("output_padding not supported!", ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_OPERATOR);
        }
        ASSIGN_VALUE_OR_RETURN_ERR(pads, getPads(dict, kernels, strides, idimHW));
        outSz = calculateConvTransposeOutputDims(idim.h, idim.w, kernels, strides, pads, dilation);
    }
    std::array<dim_t, 4> outDims = {{idim.n, outSz.first, outSz.second, depth}};
    auto outTy = mod_.uniqueType(ElemKind::FloatTy, outDims);

    auto *node = G_->createConvTranspose(opName, tr, filterTransposeNode, bias, outTy, kernels, strides, pads, group, dilation);
...
Error ONNXModelLoader::loadPool(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict, llvm::StringRef typeName) {
    const std::string &opName = loadOperatorName(op);
    ...
    ASSIGN_VALUE_OR_RETURN_ERR(kernelsShape, getShape<unsigned_t>(dict["kernel_shape"]));
    ...
    if (inDim == 3) {
        in = G_->createExpandDims(opName, in, 2);
        if (kerDim != 1) {
            RETURN_ERR("Glow handles 1D pooling with kernel dimenstion size 1", ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_SHAPE);
        } else {
            if (dict.count("strides")) {
    ...
    if (in.dims().size() != 4 || kernels.size() != 2) {
        ...
        RETURN_ERR("Glow only handles 2D pooling currently.", ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_SHAPE);
    }
    ...
    if (op.output_size() > 1) {
        if (typeName != "MaxPool") {
            RETURN_ERR("Argmax output is only supported for MaxPool!", ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_OPERATOR);
        }
...
Error ONNXModelLoader::loadTensorwiseQuantizedPool(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict, llvm::StringRef typeName) {
    const std::string &opName = loadOperatorName(op);
...
Error ONNXModelLoader::loadArgMax(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
...
Error ONNXModelLoader::loadUpsample(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {

    RETURN_ERR_IF_NOT((opsetVersion_ < 10) && (opsetVersion_ > 6), "Upsample operator is supported for opset version between 7 and 9");
    ...
    RETURN_ERR_IF_NOT(mode.compare("nearest") == 0, "Upsample Operator has nearest mode support only");
    ...
    for (auto &val : scales) {
        RETURN_ERR_IF_NOT(val >= 1, "Scales value can only be greater than or equal to 1");
    }
...
Error ONNXModelLoader::loadGlobalAveragePool(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    ...
    Pads pads;
    ASSIGN_VALUE_OR_RETURN_ERR(pads, getPads(dict, kernels, strides, kernels /* input sizes*/));
...
Error ONNXModelLoader::loadSqueeze(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
...
Error ONNXModelLoader::loadUnsqueeze(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
...
Error ONNXModelLoader::loadBatchNormalization(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    ...
    auto *node = G_->createBatchNormalization(opName, in, bias, scale, mean, var, 1, epsilon);
...
Error ONNXModelLoader::loadConcat(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
...
Error ONNXModelLoader::loadFCTransposed(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
...
Error ONNXModelLoader::loadGemm(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
...
Error ONNXModelLoader::loadMatMul(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
...
Error ONNXModelLoader::loadLeakyRelu(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    // Input Type.
    NodeValue input;
    ASSIGN_VALUE_OR_RETURN_ERR(input, getNodeValueByName(op.input(0)));
    ...
    RETURN_ERR_IF_NOT((inputType == ElemKind::FloatTy) || (inputType == ElemKind::Float16Ty), "Unsupported Type for LeakyRelu");
...
Error ONNXModelLoader::loadPad(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    ...
    if (dict.count("mode")) {
        ...
        if (modeStr == "constant") {
            ...
            RETURN_ERR("Pad: Invalid mode", ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_ATTRIBUTE);
        }
    }
    ...
    RETURN_ERR_IF_NOT((pads.size() == 2 * numDims), "Pad: the 'pads' array must contain 2 values per dimensions");
    ...
    for (unsigned_t i = 0; i < numDims; i++) {
        ...
        RETURN_ERR_IF_NOT(new_dim > 0, "The padding can't remove all elements of a dimension");
        outDims[i] = new_dim;
    }
    auto outTy = mod_.uniqueType(ElemKind::FloatTy, outDims);
...
Error ONNXModelLoader::loadCast(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    ...
    RETURN_ERR_IF_NOT(ONNX_NAMESPACE::TensorProto_DataType_IsValid(toONNXTypeValue), "Cast: invalid target type",
                      ErrorValue::ErrorCode::MODEL_LOADER_INVALID_PROTOBUF);
    ASSIGN_VALUE_OR_RETURN_ERR(targetKind, convertTensorProtoDataType(ONNX_NAMESPACE::TensorProto_DataType(toONNXTypeValue)));

    // Only support non quantized types.
    RETURN_ERR_IF_NOT((!isQuantizedElemKind(inputKind)) && (!isQuantizedElemKind(targetKind)), "Unsupported Cast");
...
Error ONNXModelLoader::loadSpaceToDepth(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
...
Error ONNXModelLoader::loadReduceL2(const ONNX_NAMESPACE::NodeProto &op, const ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    NodeValue in;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
    ...
    if (dict.count("axes")) {
        ...
        if (shapeAxes.size() > 1) {
            ...
            if (it != shapeAxes.end())
                RETURN_ERR("Axes values are not unique", ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_SHAPE);
        }
    } else {
...
Error ONNXModelLoader::loadConstantOfShape(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict, bool isSplat) {
    ...
    if (dict.count("value")) {
        ...
        if (!isSplat) {
            ...
            RETURN_ERR_IF_NOT(T.getType().getElementType() == ElemKind::FloatTy || T.getType().getElementType() == ElemKind::Int64ITy ||
    ...
    if (op.input_size() > 0) {
        ...
        RETURN_ERR_IF_NOT(in->getType()->getElementType() == ElemKind::Int64ITy, "Input element type must be Int64ITy.");
        ...
        case ElemKind::Int64ITy: {
            ...
            RETURN_ERR_IF_NOT(v == static_cast<int64_t>(static_cast<float>(v)),
                              "This ConstantOfShape implementation may cause losses for value " + std::to_string(v) + " .");
            SN = G_->createSplat(loadOperatorName(op), ty, v);
            break;
        }
        case ElemKind::Int32ITy: {
            int32_t v = T.getHandle<int32_t>().raw(0);
            RETURN_ERR_IF_NOT(v == static_cast<int32_t>(static_cast<float>(v)),
                              "This ConstantOfShape implementation may cause losses for value " + std::to_string(v) + " .");
            SN = G_->createSplat(loadOperatorName(op), ty, v);
            break;
        }
...
Error ONNXModelLoader::loadTile(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
...
Error ONNXModelLoader::loadExpand(const ONNX_NAMESPACE::NodeProto &op, const ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    NodeValue in;
    Constant *repeats;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
...
Expected<bool> ONNXModelLoader::foldOperator(const ONNX_NAMESPACE::NodeProto &op) {
    const unsigned numInputs = op.input_size();
    ...
    bool foldStatus = !ERR_TO_BOOL(constantFoldInLoader<ONNXModelLoader, ONNX_NAMESPACE::NodeProto>(tmpF, tmpLoader, this, op),
                                   /* log */ false);
    mod_.eraseFunction(tmpF);
    return foldStatus;
}

Error ONNXModelLoader::loadWhere(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue cNV;
...
static Expected<Function::RnnDirection> getRnnDirection(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
...
static Function::RnnActivation RnnActivationRelu(Function &F) {
    return [&F](llvm::StringRef name, Node *input) { return F.createRELU(name, input); };
}

/// Tanh activation function definition.
static Function::RnnActivation RnnActivationTanh(Function &F) {
    return [&F](llvm::StringRef name, Node *input) { return F.createTanh(name, input); };
}

/// Sigmoid activation function definition.
static Function::RnnActivation RnnActivationSigmoid(Function &F) {
    return [&F](llvm::StringRef name, Node *input) { return F.createSigmoid(name, input); };
}
...
static Error getRnnActivations(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict, Function *F,
                               std::vector<Function::RnnActivation> &activations) {

    // Activation alpha not supported (Optional)(Default:activation dependent).
    RETURN_ERR_IF_NOT(!dict.count("activation_alpha"), "ONNX " + op.op_type() + " 'activation_alpha' attribute not supported!");

    // Activation beta not supported (Optional)(Default:activation dependent).
    RETURN_ERR_IF_NOT(!dict.count("activation_beta"), "ONNX " + op.op_type() + " 'activation_beta' attribute not supported!");
    ...
    if (dict.count("activations") && dict.at("activations")->strings_size()) {
        ...
        RETURN_ERR_IF_NOT(actNum == actNumExpected, strFormat("ONNX %s 'activations' attribute has invalid "
        ...
        for (size_t actIdx = 0; actIdx < actNum; actIdx++) {
            ...
            if (actStr == "Relu") {
                ...
                RETURN_ERR("ONNX " + op.op_type() + " activation '" + actStr + "' not supported!",
...
Error ONNXModelLoader::loadRNN(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    ...
    dim_t numDirections = (direction == Function::RnnDirection::Bidirectional) ? 2 : 1;
    ...
    RETURN_ERR_IF_NOT(!dict.count("clip"), "ONNX RNN 'clip' attribute not supported!");

    // Get hidden size (Required).
    dim_t hiddenSize;
    RETURN_ERR_IF_NOT(dict.count("hidden_size"), "ONNX RNN 'hidden_size' attribute is required!");
    ASSIGN_VALUE_OR_RETURN_ERR(hiddenSize, loadInt(dict.at("hidden_size")));

    // --------------------------- Inputs ---------------------------------------
    const int numInputs = op.input_size();
    RETURN_ERR_IF_NOT((3 <= numInputs) && (numInputs <= 6), "ONNX RNN should have minimum 3 and maximum 6 inputs!");
    ...
    if (numInputs > 4) {
        RETURN_ERR_IF_NOT(op.input(4).empty(), "ONNX RNN 'sequence_lens' attribute not supported!");
    }
    ...
    // Derived parameters.
    RETURN_ERR_IF_NOT(X.dims().size() == 3, "ONNX RNN input 'X' should have 3 dimensions!");
    dim_t batchSize = X.dims()[1];

    // Create Y_h (hidden state) output placeholder.
    Placeholder *Y_h_ph;
    TypeRef Htype = mod_.uniqueTypeWithNewShape(X.getType(), {numDirections, batchSize, hiddenSize});
    std::string Hname = opName + ".Y_h";
    ASSIGN_VALUE_OR_RETURN_ERR(Y_h_ph, createAndRegisterPlaceholder(Hname, Htype));
    ...
    // Create ONNX RNN.
    NodeValue Y, Y_h;
    G_->createOnnxRNN(opName, X, W, R, B, Y_h_init, Y, Y_h, hiddenSize, direction, activations);
...
// - Activation clipping not supported.
// - Variable sequence length not supported.
Error ONNXModelLoader::loadGRU(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    ...
    std::vector<Function::RnnActivation> activations;
    if (direction == Function::RnnDirection::Bidirectional) {
        activations = {RnnActivationSigmoid(*G_), RnnActivationTanh(*G_), RnnActivationSigmoid(*G_), RnnActivationTanh(*G_)};
    } else {
    ...
    // Activation clipping not supported (Optional)(Default: 0 for no clipping).
    RETURN_ERR_IF_NOT(!dict.count("clip"), "ONNX GRU 'clip' attribute not supported!");

    // Get hidden size (Required).
    dim_t hiddenSize;
    RETURN_ERR_IF_NOT(dict.count("hidden_size"), "ONNX GRU 'hidden_size' attribute is required!");
    ASSIGN_VALUE_OR_RETURN_ERR(hiddenSize, loadInt(dict.at("hidden_size")));

    // Get linear_before_reset (Optional)(Default:0).
    int linearBeforeReset = 0;
    if (dict.count("linear_before_reset") && dict.at("linear_before_reset")->has_i()) {
        linearBeforeReset = dict.at("linear_before_reset")->i();
    }

    // --------------------------- Inputs ---------------------------------------
    const int numInputs = op.input_size();
    RETURN_ERR_IF_NOT((3 <= numInputs) && (numInputs <= 6), "ONNX GRU should have minimum 3 and maximum 6 inputs!");
    ...
    // Input4: sequence_lens (Optional).
    if (numInputs > 4) {
        RETURN_ERR_IF_NOT(op.input(4).empty(), "ONNX GRU 'sequence_lens' attribute not supported!");
    }
    ...
    // Derived parameters.
    RETURN_ERR_IF_NOT(X.dims().size() == 3, "ONNX GRU input 'X' should have 3 dimensions!");
    dim_t batchSize = X.dims()[1];
    ...
    // Create ONNX GRU.
    NodeValue Y, Y_h;
    G_->createOnnxGRU(opName, X, W, R, B, Y_h_init, Y, Y_h, hiddenSize, direction, activations, (bool)linearBeforeReset);
...
// - Activation clipping not supported.
// - Variable sequence length not supported.
Error ONNXModelLoader::loadLSTM(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    ...
    std::vector<Function::RnnActivation> activations;
    if (direction == Function::RnnDirection::Bidirectional) {
        activations = {RnnActivationSigmoid(*G_), RnnActivationTanh(*G_), RnnActivationTanh(*G_),
                       RnnActivationSigmoid(*G_), RnnActivationTanh(*G_), RnnActivationTanh(*G_)};
    } else {
        activations = {RnnActivationSigmoid(*G_), RnnActivationTanh(*G_), RnnActivationTanh(*G_)};
    }
    RETURN_IF_ERR(getRnnActivations(op, dict, G_, activations));
    ...
    // Activation clipping not supported (Optional)(Default: 0 for no clipping).
    RETURN_ERR_IF_NOT(!dict.count("clip"), "ONNX LSTM 'clip' attribute not supported!");

    // Get hidden size (Required).
    dim_t hiddenSize;
    RETURN_ERR_IF_NOT(dict.count("hidden_size"), "ONNX LSTM 'hidden_size' attribute is required!");
    ASSIGN_VALUE_OR_RETURN_ERR(hiddenSize, loadInt(dict.at("hidden_size")));
    ...
    // --------------------------- Inputs ---------------------------------------
    const int numInputs = op.input_size();
    RETURN_ERR_IF_NOT((3 <= numInputs) && (numInputs <= 8), "ONNX LSTM should have minimum 3 and maximum 8 inputs!");
    ...
    // Input4: sequence_lens (Optional).
    if (numInputs > 4) {
        RETURN_ERR_IF_NOT(op.input(4).empty(), "ONNX LSTM 'sequence_lens' attribute not supported!");
    }
    ...
    // Derived parameters.
    RETURN_ERR_IF_NOT(X.dims().size() == 3, "ONNX LSTM input 'X' should have 3 dimensions!");
    dim_t batchSize = X.dims()[1];
    ...
    Placeholder *Y_c_ph;
    TypeRef Ctype = mod_.uniqueTypeWithNewShape(X.getType(), {numDirections, batchSize, hiddenSize});
    std::string Cname = opName + ".Y_c";
    ASSIGN_VALUE_OR_RETURN_ERR(Y_c_ph, createAndRegisterPlaceholder(Cname, Ctype));
    ...
    // Create ONNX LSTM.
    NodeValue Y, Y_h, Y_c;
    G_->createOnnxLSTM(opName, X, W, R, B, Y_h_init, Y_c_init, P, Y, Y_h, Y_c, hiddenSize, direction, activations, (bool)inputForget);
...
Error ONNXModelLoader::loadCmpEQ(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue LHS;
    ASSIGN_VALUE_OR_RETURN_ERR(LHS, getNodeValueByName(op.input(0)));
    NodeValue RHS;
    ASSIGN_VALUE_OR_RETURN_ERR(RHS, getNodeValueByName(op.input(1)));
...
Error ONNXModelLoader::loadCmpLTE(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue LHS;
    ASSIGN_VALUE_OR_RETURN_ERR(LHS, getNodeValueByName(op.input(0)));
    NodeValue RHS;
    ASSIGN_VALUE_OR_RETURN_ERR(RHS, getNodeValueByName(op.input(1)));
...
Error ONNXModelLoader::loadSelect(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue Cond;
...
Error ONNXModelLoader::loadQuantize(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue in;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
...
Error ONNXModelLoader::loadConvertTo(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue in;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));

    const auto *attr = dict.at("shape");
    RETURN_ERR_IF_NOT(attr->has_t(), "ConvertTo should have t() field as \"shape\"");
    const auto &t = attr->t();
    std::vector<dim_t> shape;
...
Error ONNXModelLoader::loadDequantize(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue in;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
...
Error ONNXModelLoader::loadRegression(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue in;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
    NodeValue expected;
...
Error ONNXModelLoader::loadBatchedAdd(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue batch;
...
Error ONNXModelLoader::loadCumSum(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    if (op.input_size() > 1) {
        Expected<NodeValue> axis = getNodeValueByName(op.input(1));
        if (axis) {
            if (auto *AC = llvm::dyn_cast<Constant>(axis->getNode())) {
                RETURN_ERR_IF_NOT(AC->getPayload().dims().size() == 1, "CumSum axis must be 0-D");
                RETURN_ERR_IF_NOT(AC->getPayload().dims()[0] == 1, "CumSum axis must be 0-D");
                RETURN_ERR_IF_NOT(AC->getHandle<int32_t>().at(0) == 0, "CumSum only supports axis == 0");
            } else {
...
Error ONNXModelLoader::loadScatterAssign(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue data;
    ASSIGN_VALUE_OR_RETURN_ERR(data, getNodeValueByName(op.input(0)));
    NodeValue indices;
...
Error ONNXModelLoader::loadIntLookupTable(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue in;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
...
Error ONNXModelLoader::loadLengthsRangeFill(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue lengths;
...
Error ONNXModelLoader::loadRescaleQuantized(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue in;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
    float scale;
    ASSIGN_VALUE_OR_RETURN_ERR(scale, loadFloat(dict.at("scale")));
    ...
    auto outTy = mod_.uniqueType(inTy->getElementType(), inTy->dims(), scale, offset);
...
Error ONNXModelLoader::loadRowwiseQuantizedSparseLengthsWeightedSum(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    Constant *data;
    ...
    Node *N = G_->createRowwiseQuantizedSparseLengthsWeightedSum(loadOperatorName(op), data, scales, offsets, weights, indices, lengths,
                                                                 /* precision */ ElemKind::FloatTy, /* useFP16Accumulation */ false, lengthsMode);

    RETURN_IF_ERR(addNodeAsOutput(op, N));
    return Error::success();
}

Error ONNXModelLoader::loadFusedRowwiseQuantizedSparseLengthsWeightedSum(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue data;
    ASSIGN_VALUE_OR_RETURN_ERR(data, getNodeValueByName(op.input(0)));
    NodeValue weights;
    ...
    Node *N = G_->createFusedRowwiseQuantizedSparseLengthsWeightedSum(loadOperatorName(op), data, weights, indices, lengths,
...
Error ONNXModelLoader::loadFusedRowwiseQuantizedSparseLengthsSum(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue data;
    ASSIGN_VALUE_OR_RETURN_ERR(data, getNodeValueByName(op.input(0)));
    NodeValue indices;
    ASSIGN_VALUE_OR_RETURN_ERR(indices, getNodeValueByName(op.input(1)));
    ...
    Node *N = G_->createFusedRowwiseQuantizedSparseLengthsSum(loadOperatorName(op), dataS, indices, lengths,
                                                              /* useFP16Accumulation */ false, lengthsMode);
...
Error ONNXModelLoader::loadFullyConnected(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue in;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
    Constant *W;
    ASSIGN_VALUE_OR_RETURN_ERR(W, getConstantByName(op.input(1)));
    ...
    Node *N = G_->createFullyConnected(loadOperatorName(op), in, W, B ? B : b, axis);

    RETURN_IF_ERR(addNodeAsOutput(op, N));
    return Error::success();
}

Error ONNXModelLoader::loadRowwiseQuantizedFullyConnected(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue input;
    ASSIGN_VALUE_OR_RETURN_ERR(input, getNodeValueByName(op.input(0)));
    ...
    auto outTy = mod_.uniqueType(ElemKind::Int8QTy, {input.dims()[0], weights.dims()[0]}, outScale, outOffset);

    Node *N = G_->createRowwiseQuantizedFullyConnected("rowwise_quantized_fc", input, weightsC, scalesC, offsetsC, biasC, outTy);

    return addNodeAsOutput(op, N);
}

Error ONNXModelLoader::loadNonMaxSuppression(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict, bool isV4) {
    NodeValue boxesNV;
    ...
    if (dict.count("center_point_box")) {
        ASSIGN_VALUE_OR_RETURN_ERR(centerPointBox, loadInt(dict.at("center_point_box")));
    }

    int32_t padToMaxOutputSize = 0;
    if (isV4) {
        if (dict.count("pad_to_max_output_size")) {
            ASSIGN_VALUE_OR_RETURN_ERR(padToMaxOutputSize, loadInt(dict.at("pad_to_max_output_size")));
        }

        // Does it make sense within GLOW context to have no padding? Since Size has
        // to be compile time constant.
        RETURN_ERR_IF_NOT(padToMaxOutputSize == 1, "NonMaxSuppressionV4 does not support non-padding mode.");
    }
    ...
    if (maxOutputBoxesPerClassC) {
        if (maxOutputBoxesPerClassC->getPayload().getElementType() == ElemKind::Int64ITy) {
            maxOutputBoxesPerClass = maxOutputBoxesPerClassC->getPayload().getHandle<int64_t>().raw(0);
        } else if (maxOutputBoxesPerClassC->getPayload().getElementType() == ElemKind::Int32ITy) {
            maxOutputBoxesPerClass = maxOutputBoxesPerClassC->getPayload().getHandle<int32_t>().raw(0);
        } else {
    ...
    if (isV4) {
        N = G_->createNonMaxSuppressionV4(opName, boxesNV, scoresNV, centerPointBox, maxOutputBoxesPerClass, iouThreshold, scoreThreshold);
    } else {
        N = G_->createNonMaxSuppressionONNX(opName, boxesNV, scoresNV, centerPointBox, maxOutputBoxesPerClass, iouThreshold, scoreThreshold);
    }
    RETURN_IF_ERR(addNodeAsOutput(op, N));
    return Error::success();
}

Error ONNXModelLoader::loadSplat(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    return loadConstantOfShape(op, dict, true /* isSplat */);
}

Error ONNXModelLoader::loadInsertTensor(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue big;
    ...
    Node *N = G_->createInsertTensor(loadOperatorName(op), big, small, start, count, axis);

    RETURN_IF_ERR(addNodeAsOutput(op, N));
    return Error::success();
}

Error ONNXModelLoader::loadIdentity(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue in;
    ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
    RETURN_IF_ERR(addNodeAsOutput(op, in));
    return Error::success();
...
Error ONNXModelLoader::loadAdaptiveAvgPool(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
    ...
    ASSIGN_VALUE_OR_RETURN_ERR(outputShape, getShape<unsigned_t>(dict["output_size"]));

    ShapeNHWC idim(input.dims());

    auto outTy = mod_.uniqueTypeWithNewShape(input.getType(), {idim.n, outputShape[0], outputShape[1], idim.c});
...
Error ONNXModelLoader::loadFlip(const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    NodeValue input;
    ASSIGN_VALUE_OR_RETURN_ERR(input, getNodeValueByName(op.input(0)));
...
Expected<TypeRef> ONNXModelLoader::loadTypeFromAttributes(unsigned resNo, ArgumentDictionaryTy &dict) {
    Module &mod = *G_->getParent();

    // Load ElemKind.
    std::string elemKindStr;
    ASSIGN_VALUE_OR_RETURN_ERR(elemKindStr, loadStr(dict[getTypeAttrID(resNo, elemKindSignifier)]));
    ...
    std::vector<dim_t> shape;
    ASSIGN_VALUE_OR_RETURN_ERR(shape, getShape<dim_t>(dict[getTypeAttrID(resNo, shapeSignifier)],
    ...
    // quantized Type.
    float scale;
    ASSIGN_VALUE_OR_RETURN_ERR(scale, loadFloat(dict[getTypeAttrID(resNo, qScaleSignifier)]));
    int32_t offset;
    ASSIGN_VALUE_OR_RETURN_ERR(offset, loadInt(dict[getTypeAttrID(resNo, qOffsetSignifier)]));
    return mod.uniqueType(k, shape, scale, offset);
}

Expected<Node *> ONNXModelLoader::tryLoadGlowCustomOp(llvm::StringRef typeName, const ONNX_NAMESPACE::NodeProto &op, ArgumentDictionaryTy &dict) {
    const std::string &opName = loadOperatorName(op);
...
static Error loadPerNodeOptions(const Node *loadedNode, llvm::StringMap<std::vector<std::string>> &nodeOpts, ArgumentDictionaryTy &dict) {
    ...
    for (const auto &attrPair : dict) {
        ...
        RETURN_ERR_IF_NOT(attr->strings_size() > 0, strFormat("%s in %s has no strings", attrPair.first.c_str(), loadedNode->getName().data()));
...
Error ONNXModelLoader::loadOperator(const ONNX_NAMESPACE::NodeProto &op) {
    ...
    if (useGlowCustomOps_) {
        Node *loadedNode;
        ASSIGN_VALUE_OR_RETURN_ERR(loadedNode, tryLoadGlowCustomOp(typeName, op, dict));
        if (loadedNode) {
            if (!perNodeOpts_) {
                return Error::success();
            }
            return loadPerNodeOptions(loadedNode, (*perNodeOpts_)[loadedNode->getParent()][loadedNode], dict);
        }
        ...
        if (typeName != "Identity") {
            RETURN_ERR("Failed to load operator " + typeName + " .", ErrorValue::ErrorCode::MODEL_LOADER_UNSUPPORTED_OPERATOR);
        }
    }

    // Check if operator is supported in parent class, CommonOperatorLoader.
    bool tryLoadCommonOperatorResult;
    ASSIGN_VALUE_OR_RETURN_ERR(tryLoadCommonOperatorResult, tryLoadCommonOperator(typeName, op, dict));
    ...
    if (typeName == "Conv") {
        ...
        NodeValue in;
        ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
        return in.getType()->isQuantizedType() ? loadTensorwiseQuantizedConvolution(op, dict) : loadConv(op, dict);
    }
    ...
    if (typeName == "MaxPool" || typeName == "AveragePool") {
        ...
        NodeValue in;
        ASSIGN_VALUE_OR_RETURN_ERR(in, getNodeValueByName(op.input(0)));
        return in.getType()->isQuantizedType() ? loadTensorwiseQuantizedPool(op, dict, typeName) : loadPool(op, dict, typeName);
    }
...
Error ONNXModelLoader::setOutputNodes(ONNX_NAMESPACE::GraphProto &net) {
    ...
    for (int i = 0; i < net.output_size(); i++) {
        ...
        Expected<std::string> saveName = getAttrFromDocString(saveNameSignifier, docString);

        const bool hasSpecifiedSaveName = !ERR_TO_BOOL(saveName.takeError(), /* log */ false);
        const std::string &saveNodeName = hasSpecifiedSaveName ? saveName.get() : outputName;

        std::string isTrainable = "0";
        std::string layout = ANY_LAYOUT;
        if (useGlowCustomOps_) {
            ASSIGN_VALUE_OR_RETURN_ERR(isTrainable, getAttrFromDocString(trainableSignifier, docString));
            ASSIGN_VALUE_OR_RETURN_ERR(layout, getAttrFromDocString(layoutSignifier, docString));
        }

        Placeholder *placeholder = mod_.createPlaceholder(r.getType(), outputName, isTrainable != "0", layout);
        SaveNode *SN = G_->createSave(saveNodeName, r, placeholder, hasSpecifiedSaveName);
    ...
    for (int i = 0; i < net.node_size(); i++) {
        ...
        if (constFoldInLoader_) {
            ...
            if (!tryFold) {
                ...
                LOG(INFO) << "Error while trying to ConstantFold " << loadOperatorName(op) << ": " << errStr;
            } else if (tryFold.get()) {
...
ONNXModelLoader::ONNXModelLoader(Function &F, Error *errPtr) : CommonOperatorLoader({}, {}, &F, errPtr) { deleteUnusedConstants(); }
...
Error ONNXModelLoader::collectStaticInputs(ONNX_NAMESPACE::GraphProto &net) {
    for (int i = 0; i < net.input_size(); i++) {
        ...
        if (useGlowCustomOps_) {
            std::string isStatic;
            ASSIGN_VALUE_OR_RETURN_ERR(isStatic, getAttrFromDocString(staticSignifier, valueInfo.doc_string()));
            if (isStatic == "1") {
                staticInputs_.emplace(inputName);
            }
        } else if (valueInfo.has_doc_string() && valueInfo.doc_string() == staticSignifier) {
            staticInputs_.emplace(inputName);
        }
    }
    return Error::success();
...
Error ONNXModelLoader::checkInputs(ONNX_NAMESPACE::GraphProto &net, llvm::ArrayRef<const char *> tensorNames, llvm::ArrayRef<TypeRef> types) {
    for (size_t i = 0; i < tensorNames.size(); i++) {
        ...
        for (int j = 0; j < net.input_size(); j++) {
            ...
            for (size_t k = 1; k < dims.size(); k++) {
                printf("\ndims: [%d,%d,%d,%d]\ndimsProto: [%d,%d,%d,%d]", dims[0], dims[1], dims[2], dims[3], dimsProto[0], dimsProto[1], dimsProto[2], dimsProto[3])
                RETURN_ERR_IF_NOT(dims[k] == dimsProto[k], "Mismatch between input image and ONNX input shape");
            }

            if (valueInfo.has_doc_string() && valueInfo.doc_string() == staticSignifier) {
                staticInputs_.emplace(inputName);
            }
        }
    }
...
ONNXModelLoader::ONNXModelLoader(const std::string &modelDescFilename, llvm::ArrayRef<const char *> tensorNames, llvm::ArrayRef<TypeRef> types,
                                 Function &F, Error *errPtr, bool zipMode, BackendSpecificNodeInfo *perNodeOpts, bool disableConstFoldInLoader,
                                 const Backend *B)
    : CommonOperatorLoader(tensorNames, types, &F, errPtr), perNodeOpts_(perNodeOpts) {
...
ONNXModelLoader::ONNXModelLoader(const void *model, uint32_t modelSize, uint32_t weightsCount, const onnxTensorDescriptorV1 *weightDescriptors,
                                 Function &F, bool loadInputsAsPlaceholders, Error *errPtr, bool constFoldInLoader)
