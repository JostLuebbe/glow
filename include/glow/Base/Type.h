...
struct ShapeNHWC {
    ...
    ShapeNHWC(dim_t samples, dim_t height, dim_t width, dim_t channels) : n(samples), h(height), w(width), c(channels) {}

    bool equals(const ShapeNHWC &other) const { return n == other.n && h == other.h && w == other.w && c == other.c; }
};
...
struct ShapeNTHWC {
    ...
    ShapeNTHWC(dim_t samples, dim_t temporal_frames, dim_t height, dim_t width, dim_t channels)
        : n(samples), t(temporal_frames), h(height), w(width), c(channels) {}

    bool equals(const ShapeNTHWC &other) const { return n == other.n && t == other.t && h == other.h && w == other.w && c == other.c; }
};
...
struct ShapeNHWTC {
    ...
    ShapeNHWTC(size_t samples, size_t height, size_t width, size_t temporal_frames, size_t channels)
        : n(samples), h(height), w(width), t(temporal_frames), c(channels) {}

    bool equals(const ShapeNHWTC &other) const { return n == other.n && h == other.h && w == other.w && t == other.t && c == other.c; }
};
...
struct ShapeNCHW {
    ...
    ShapeNCHW(dim_t samples, dim_t channels, dim_t height, dim_t width) : n(samples), c(channels), h(height), w(width) {}

    bool equals(const ShapeNCHW &other) const { return n == other.n && h == other.h && w == other.w && c == other.c; }
};
...
struct ShapeNCTHW {
    ...
    ShapeNCTHW(dim_t samples, dim_t channels, dim_t temporal_frames, dim_t height, dim_t width)
        : n(samples), c(channels), t(temporal_frames), h(height), w(width) {}

    bool equals(const ShapeNCTHW &other) const { return n == other.n && t == other.t && h == other.h && w == other.w && c == other.c; }
};
...
struct PaddingTLBR {
    ...
    bool equalPadding() const { return top == left && top == bottom && top == right; }
};
...
struct PaddingTLNBRF {
    ...
    bool equalPadding() const { return top == left && top == bottom && top == right && top == near && top == far; }
};
...
inline std::pair<dim_t, dim_t> flattenCdr(llvm::ArrayRef<dim_t> dims, unsigned_t n = 1) {
...
inline bool operator==(const ShapeNHWC &LHS, const ShapeNHWC &RHS) { return LHS.equals(RHS); }

inline bool operator==(const ShapeNCHW &LHS, const ShapeNCHW &RHS) { return LHS.equals(RHS); }

inline bool operator==(const ShapeNHWTC &LHS, const ShapeNHWTC &RHS) { return LHS.equals(RHS); }
...
inline bool isQuantizedElemKind(ElemKind e) {
    return e == ElemKind::Int8QTy || e == ElemKind::UInt8QTy || e == ElemKind::Int16QTy || e == ElemKind::Int32QTy || e == ElemKind::UInt8FusedQTy ||
           e == ElemKind::UInt8FusedFP16QTy || e == ElemKind::UInt4FusedFP16QTy;
}

/// \returns whether \p e is a float ElemKind.
inline bool isFloatElemKind(ElemKind e) { return e == ElemKind::FloatTy || e == ElemKind::Float16Ty; }

/// \returns whether \p e is a fused quantized ElemKind.
inline bool isFusedQuantizedElemKind(ElemKind e) {
    return e == ElemKind::UInt8FusedQTy || e == ElemKind::UInt8FusedFP16QTy || e == ElemKind::UInt4FusedFP16QTy;
}
...
struct Type final {
    ...
    Type(ElemKind elemTy, llvm::ArrayRef<dim_t> dims, float scale, int32_t offset) : scale_(scale), offset_(offset), elementType_(elemTy) {
    ...
    Type(ElemKind elemTy, llvm::ArrayRef<dim_t> dims) : elementType_(elemTy) {
        assert(!isQuantizedType() && "Can't initialize quantized types without scale and offset");
        ShapeVector alignments(dims.size(), 1);
        initDims(dims, llvm::makeArrayRef(alignments));
    }

    /// Initialize a new quantized type with \p scale and \p offset.
    Type(ElemKind elemTy, llvm::ArrayRef<dim_t> dims, llvm::ArrayRef<dim_t> alignments, float scale, int32_t offset)
    ...
    /// Initialize a new non-quantized type.
    Type(ElemKind elemTy, llvm::ArrayRef<dim_t> dims, llvm::ArrayRef<dim_t> alignments) : elementType_(elemTy) {
        assert(!isQuantizedType() && "Can't initialize quantized types without scale and offset");
        initDims(dims, alignments);
    }
    ...
    static Type newShape(const Type &T, llvm::ArrayRef<dim_t> dims, llvm::ArrayRef<dim_t> alignments) {
        if (T.isQuantizedType()) {
            return Type(T.getElementType(), dims, alignments, T.getScale(), T.getOffset());
        } else {
    ...
    static Type newShape(const Type &T, TypeRef shapeType) {
        assert(T.getElementSize() == shapeType->getElementSize() && "Element size should be the same");
        Type ty;
        if (T.isQuantizedType()) {
            ty = Type(T.getElementType(), shapeType->dims(), T.getScale(), T.getOffset());
        } else {
            ty = Type(T.getElementType(), shapeType->dims());
        }
        // Copy the stride information.
        std::copy(&shapeType->strides_[0], &shapeType->strides_[ty.numSizes_], ty.strides_);
        return ty;
    }
    ...
    std::pair<float, float> getQuantizedValueRange() const {
        assert(isQuantizedType() && "Can't get the quantized value range of a non-quantized type");
    ...
    bool isEqual(const Type &other, bool allowDifferentShape = false, bool allowDifferentStrides = false) const {
    ...
    llvm::hash_code equals_hash() const {
        return llvm::hash_combine(elementType_, dims(),
    ...
    template <class ElemTy> bool isType() const { return isType<ElemTy>(elementType_); }
    ...
    /// types.
    bool isFPType() const { return getElementType() == ElemKind::FloatTy || getElementType() == ElemKind::Float16Ty; }
    ...
    size_t getSizeInBytes() const {
        ...
        for (unsigned char i = 0; i < numSizes_; i++) {
            s = std::max<dim_t>(s, size_t(sizes_[i]) * getElementSize() * strides_[i]);
        }
        return s;
    }
    ...
    llvm::StringRef getElementName() const { return getElementName(elementType_); }

    /// \return the textual name of the element \p Ty.
    static llvm::StringRef getElementName(ElemKind Ty) {
        static const char *names[] = {
            "float", "float16", "i8", "ui8", "i16", "i32", "index32", "index64", "ui8fused", "ui8fusedfp16", "ui4fusedfp16", "bool",
        };
        return names[(int)Ty];
    }
  ...
  private:
    ...
    void initDims(llvm::ArrayRef<dim_t> dims, llvm::ArrayRef<dim_t> alignments) {
        ...
        assert(dims.size() == alignments.size() && "The number of dimensions and alignments should be the same");
        ...
        if (numSizes_ > 0) {
            ...
            assert(alignments[numSizes_ - 1] == 1 && "Last dimension must always be aligned.");
        ...
        for (int i = numSizes_ - 2; i >= 0; i--) {
            ...
            if (alignment != 1) {
                assert(alignment % getElementSize() == 0 && "Alignment should be a multiple of element size");
...
inline bool operator==(const Type &LHS, const Type &RHS) { return LHS.isEqual(RHS); }
